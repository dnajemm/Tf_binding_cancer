...
motif_probe_file <- "./methylation/correlation_expression_methylation/tumor_vs_healthy/BANP/BANP_proximal_motif_probe_map_long.tsv.gz"
expr_file        <- "./expression/gene_expression_matrix_3d4d_noOV.tsv"
expr_annot_file  <- "./methylation/correlation_expression_methylation/tumor_vs_healthy/BANP/expression_sample_annotation_all.tsv.gz"
meth_file        <- "./methylation/correlation_expression_methylation/tumor_vs_healthy/BANP/motif_sample_methylation_all.tsv.gz"
out_dir          <- "./methylation/correlation_expression_methylation/tumor_vs_healthy/BANP/"
...
# INPUTS
# =========================
in_file  <- "./methylation/correlation_expression_methylation/tumor_vs_healthy/BANP/motif_sample_expression_methylation_all.tsv.gz"
out_dir  <- "./methylation/correlation_expression_methylation/tumor_vs_healthy/BANP/correlation_stats_by_cancer"
...
# INPUTS
# =========================
data_file   <- "./methylation/correlation_expression_methylation/tumor_vs_healthy/BANP/motif_sample_expression_methylation_all.tsv.gz"
stats_file  <- "./methylation/correlation_expression_methylation/tumor_vs_healthy/BANP/correlation_stats_by_cancer/pearson_correlation_all_pairs_by_cancer.tsv.gz"
color_file  <- "./results/multi_omics/cancer_color_order_with_defined_colours.tsv"
out_dir     <- "./results/methylation/correlation_expression_methylation_2/tumor_vs_healthy/BANP/top_pair_plots"
...
# 8) plot the heatmap of the correlation of all pairs across all cancers (from the stats file) : one heatmap per cancer type, with genes on the x axis and motifs on the y axis, colored by the Pearson correlation coefficient (r) and annotated with stars for significance (FDR < 0.05)
Rscript - <<'EOF'
suppressPackageStartupMessages({
  library(data.table)
  library(pheatmap)
...
# =========================
stats_file <- "./methylation/correlation_expression_methylation/tumor_vs_healthy/BANP/correlation_stats_by_cancer/pearson_correlation_all_pairs_by_cancer.tsv.gz"
out_pdf    <- "./results/methylation/correlation_expression_methylation_2/tumor_vs_healthy/BANP/correlation_heatmap_pairs_vs_cancers.pdf"
...
need <- c("cancer", "gene", "motif_id", "pearson_r", "padj_BH")
...
# keep one row per cancer/gene/motif if duplicates exist
st[, abs_r := abs(pearson_r)]
setorder(st, cancer, gene, motif_id, padj_BH, -abs_r)
st <- st[, .SD[1], by = .(cancer, gene, motif_id)]
st[, abs_r := NULL]

# labels
st[, stars := p_to_stars(padj_BH)]
st[, pair_id := paste(gene, motif_id, sep = " | ")]

# optional ordering: strongest pairs first
pair_order <- st[, .(
  max_abs_r = max(abs(pearson_r), na.rm = TRUE),
  min_padj  = suppressWarnings(min(padj_BH, na.rm = TRUE))
), by = pair_id][order(-max_abs_r, min_padj)]$pair_id

cancer_order <- sort(unique(st$cancer))

st[, pair_id := factor(pair_id, levels = pair_order)]
st[, cancer := factor(cancer, levels = cancer_order)]

# =========================
# MATRICES
# =========================
mat_r <- dcast(st, pair_id ~ cancer, value.var = "pearson_r", fill = NA_real_)
mat_s <- dcast(st, pair_id ~ cancer, value.var = "stars", fill = "")

mat_r_m <- as.matrix(mat_r[, -1, with = FALSE])
rownames(mat_r_m) <- mat_r$pair_id

mat_s_m <- as.matrix(mat_s[, -1, with = FALSE])
rownames(mat_s_m) <- mat_s$pair_id

# =========================
# PLOT
# =========================
pdf(out_pdf, width = 9, height = max(10, nrow(mat_r_m) * 0.18), onefile = TRUE)

pheatmap(
  mat_r_m,
  cluster_rows = FALSE,
  cluster_cols = FALSE,
  display_numbers = mat_s_m,
  number_color = "black",
  fontsize_number = 8,
  fontsize_row = 6,
  fontsize_col = 10,
  angle_col = 45,
  main = "Motif-gene pair correlation across cancer types",
